module m_userfile
  ...
  integer, private :: cs_lecs, cs_ions, cs_poss, up_lecs, up_ions, up_poss
  real, private :: b_guide, f_ions
  !...............................................................!
...
contains
  ...
  subroutine userReadInput()
    implicit none
    ! ion mixture = n_i / (n_i + n_+)
    call getInput('problem', 'f_ions', f_ions, 0.0)
    ...
    call getInput('problem', 'cs_poss', cs_poss, 2)
    call getInput('problem', 'cs_ions', cs_ions, 3)

    ! upstream
    call getInput('problem', 'up_temperature', up_temperature)
    call getInput('problem', 'up_lecs', up_lecs, 4)
    call getInput('problem', 'up_poss', up_poss, 5)
    call getInput('problem', 'up_ions', up_ions, 6)
  ...
  subroutine userInitParticles()
    implicit none
    real :: nUP_lecs, nUP_poss, nCS_lecs, nCS_poss, nUP_ions, nCS_ions, mu_ions
    ...
    ! electrons
    nUP_lecs = 0.5 * ppc0
    nCS_lecs = nUP_lecs * cs_overdensity

    ! ions
    nUP_ions = nUP_lecs * f_ions
    nCS_ions = nCS_lecs * f_ions
    mu_ions = species(cs_ions) % m_sp

    ! positrons
    nUP_poss = nUP_lecs * (1.0 - f_ions)
    nCS_poss = nCS_lecs * (1.0 - f_ions)
...
#endif
    if (f_ions .ne. 1.0) then
      call fillRegionWithThermalPlasma(back_region, (/up_lecs, up_poss/), 2, nUP_poss, up_temperature)
    end if
    if (f_ions .ne. 0.0) then
      call fillRegionWithThermalPlasma(back_region, (/up_lecs, up_ions/), 2, nUP_ions, up_temperature)
    end if

    if (cs_overdensity .ne. 0.0) then
      ...
      current_sheet_T = 0.5 * sigma * (1.0 + b_guide**2) / &
                        (cs_overdensity * (1.0 - 0.5 * f_ions + 0.5 * f_ions * mu_ions))

      cs_region % x_min = sx_glob * 0.5 - 10.0 * cs_width
      cs_region % x_max = sx_glob * 0.5 + 10.0 * cs_width
      cs_region % y_min = 0.0
      cs_region % y_max = sy_glob
#ifdef threeD
      cs_region % z_min = 0.0
      cs_region % z_max = sz_glob
#endif
      if (f_ions .ne. 1.0) then
        call fillRegionWithThermalPlasma(cs_region, (/cs_lecs, cs_poss/), 2, nCS_poss, current_sheet_T, &
                                         shift_gamma=shift_gamma, shift_dir=3, &
                                         spat_distr_ptr=spat_distr_ptr, &
                                         dummy1=0.5 * sx_glob, dummy2=cs_width, dummy3=0.5 * sy_glob)
      end if
      if (f_ions .ne. 0.0) then
        call fillRegionWithThermalPlasma(cs_region, (/cs_lecs, cs_ions/), 2, nCS_ions, current_sheet_T, &
                                         shift_gamma=shift_gamma, shift_dir=3, &
                                         spat_distr_ptr=spat_distr_ptr, &
                                         dummy1=0.5 * sx_glob, dummy2=cs_width, dummy3=0.5 * sy_glob)
      end if
  ...
  subroutine userParticleBoundaryConditions(step)
    ...
    if (f_ions .ne. 1.0) then
      call computeNpart(up_poss, reset=.false., ds=0)
    end if
    if (f_ions .ne. 0.0) then
...
#endif
        if (f_ions .ne. 1.0) then
          call fillRegionWithThermalPlasma(imin_inj_region, (/up_lecs, up_poss/), 2, &
                                           0.5 * REAL(ppc0 - dens_imin) * (1.0 - f_ions), up_temperature)
        end if
        if (f_ions .ne. 0.0) then
          call fillRegionWithThermalPlasma(imin_inj_region, (/up_lecs, up_ions/), 2, &
                                           0.5 * REAL(ppc0 - dens_imin) * f_ions, up_temperature)
        end if
      end if
    end if
...
#endif
        if (f_ions .ne. 1.0) then
          call fillRegionWithThermalPlasma(imax_inj_region, (/up_lecs, up_poss/), 2, &
                                           0.5 * REAL(ppc0 - dens_imax) * (1.0 - f_ions), up_temperature)
        end if
        if (f_ions .ne. 0.0) then
          call fillRegionWithThermalPlasma(imax_inj_region, (/up_lecs, up_ions/), 2, &
                                           0.5 * REAL(ppc0 - dens_imax) * f_ions, up_temperature)
        end if
      end if
    end if
